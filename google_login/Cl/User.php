...
class Cl_User
{
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "SELECT * FROM pelapor where email = '$email' and password = '$password' ";
/*			print_r($query);
			die();*/
...
if($conn->connect_error){
	echo "Gagal Koneksi";
		...
		$query = "SELECT * FROM pelapor where email = '$email' and social_id = '$social_id' ";
		$result = mysqli_query($this->_con, $query);
		$data = mysqli_fetch_assoc($result);
		...
		if( $count == 1){
			...
			$query = "INSERT INTO pelapor (id_pelapor, nama_pelapor, email, social_id, picture, created) VALUES ('$idBaru', '$name', '$email', '$social_id', '$picture', CURRENT_TIMESTAMP)";
			if(mysqli_query($this->_con, $query));
			$query = "SELECT * FROM pelapor where email = '$email' and social_id = '$social_id' ";
			$result = mysqli_query($this->_con, $query);
			$data = mysqli_fetch_assoc($result);
	...
	public function twitter_login( array $data )
	{
		...
		if( $count == 1){
			...
			$query = "INSERT INTO users (user_id, password, name, email, social_id, picture, created) VALUES (NULL, '$password1', '$name', '$email', '$social_id', '$picture', CURRENT_TIMESTAMP)";
